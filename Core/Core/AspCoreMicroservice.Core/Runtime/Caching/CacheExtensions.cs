...
namespace AspCoreMicroservice.Core.Runtime.Caching
{
    ...
    public static class CacheExtensions
    {
        ...
        public static ITypedCache<TValue> AsTyped<TValue>(this ICache cache)
        {
            return new TypedCacheWrapper<TValue>(cache);
        }
        ...
        {
            var value = await cache.GetAsync(key.ToString(), async (keyAsString) => await factory(key));
